...
async def account_health_single(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        async with Client("hc_single", api_id=_API_ID, api_hash=_API_HASH,
            ...
            try:
                refreshed_session = await c.export_session_string()
            except Exception:
                refreshed_session = identifier
    ...
    if new_status == "active" and refreshed_session:
        try:
            update_account_session(user_id, acc, refreshed_session)
        except Exception:
            logger.exception("Could not persist refreshed session for %s", identifier)
