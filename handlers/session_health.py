            ...
            async def verify_once() -> dict[str, str]:
                ...
                async with Client(
                    ...
                    try:
                        ...
                        # Verification must still succeed if exporting is not
                        # available in a particular Pyrogram build.
                        refreshed_session = session_string
                    return {
                        "name": full_name,
                        "username": f"@{me.username}" if me.username else "",
                        "phone": f"+{me.phone_number}" if me.phone_number else "",
                        "tg_id": str(me.id) if me.id else "",
                        # The verifier uses an in-memory client. Persisting
                        # the current export prevents a reconnect/DC migration
                        # from leaving PostgreSQL with an older session.
                        "session_string": refreshed_session,
                        "error": "",
                    }
...
async def session_health_check(
    ...
    from storage import set_account_status, update_account_session
    ...
    for user_id, account, status, info in checked:
        ...
        if status == STATUS_ACTIVE:
            ...
            refreshed_session = info.get("session_string", "")
            if refreshed_session:
                try:
                    update_account_session(user_id, account, refreshed_session)
                except Exception:
                    logger.exception(
                        "Could not persist refreshed session for %s",
                        identifier,
                    )
