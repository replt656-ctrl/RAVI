...
def _referral_message_links(message: Any) -> list[str]:
    """Collect channel/invite URLs from text and inline keyboard buttons."""
    values: list[str] = []
...
def _referral_verify_buttons(message: Any) -> list[tuple[str, str]]:
    """Return callback buttons that look like a post-join verification step."""
    verify_words = (
        'verify', 'verification', 'check', 'joined', 'continue',
        'confirm', 'done', 'complete', 'claim',
    )
    markup = getattr(message, 'reply_markup', None)
    ...
    for row in getattr(markup, 'inline_keyboard', None) or []:
        for button in row:
            ...
            if callback_data is not None and any(
                word in label for word in verify_words
            ):
                if isinstance(callback_data, bytes):
...
async def _wait_for_referral_replies(
    ...
    while time.monotonic() < deadline:
        try:
            async for message in client.get_chat_history(bot_username, limit=30):
                if (
...
async def _run_bot_referral(client, bot_username: str, payload: str | None) -> dict:
    ...
    verify_buttons: list[tuple[Any, str, str]] = []
    for message in replies:
        ...
        for label, callback_data in _referral_verify_buttons(message):
            verify_buttons.append((message, label, callback_data))
    ...
    for message, label, callback_data in verify_buttons[:3]:
        try:
            ...
            verify_errors.append(f'{label or "verify"}: {exc}')

    # Give the bot a short opportunity to send a follow-up after verification.
    if verified:
        follow_up = await _wait_for_referral_replies(
            client,
            bot_username,
            max(getattr(message, 'id', 0) for message in replies),
            timeout=5.0,
        )
                ...
                if target and target not in join_targets:
                    ...
                    try:
                        ...
                        if 'USER_ALREADY_PARTICIPANT' not in str(exc).upper():
                            join_errors.append(f'{target}: {exc}')
    ...
    if verify_errors and verified == 0 and verify_buttons:
        return {
            'ok': False,
            'error': 'Referral verification failed: ' + '; '.join(verify_errors[:3]),
            'fatal': False,
        }
    if not join_targets and not verify_buttons:
        return {
            'ok': False,
            'error': (
    ...
    if verify_buttons:
        details.append(f'verified {verified} button(s)')
    if join_errors:
    ...
    try:
        async with client:
            if action in {'dm', 'bulk_dm'}:
                ...
                return await _run_bot_referral(client, bot_username, payload)
            ...
            # ``in_memory=True`` prevents Pyrogram from persisting changes on
            # disk. Export the current state while connected so a later
            # restart does not fall back to a stale pre-migration session.
            try:
                refreshed_session = await client.export_session_string()
            except Exception:
                logger.debug(
                    'Could not refresh session for account %s',
                    identifier,
                    exc_info=True,
                )

        if refreshed_session and refreshed_session != session_str:
            try:
                import storage
                storage.update_account_session(
                    user_id,
                    acc,
                    refreshed_session,
                )
                ...
                logger.warning(
                    'Could not persist refreshed session for account %s',
                    identifier,
                    exc_info=True,
                )
