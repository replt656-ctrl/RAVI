...
REFERRAL_MAX_ROUNDS = int(os.environ.get('REFERRAL_MAX_ROUNDS', '20'))
...
def parse_post_url(url: str) -> tuple:
    ...
    # Accept numeric channel targets copied as either
    # ``-100<channel_id>/<message_id>`` or ``<channel_id>/<message_id>``.
    # This is useful for campaigns where the channel has no public username.
    if len(parts) >= 2 and re.fullmatch(r'-?\d+', parts[0]) and parts[1].isdigit():
...
def normalize_numeric_channel_target(target):
    """Normalize a positive bare ID when the caller explicitly expects a channel."""
    peer = parse_chat_target(target)
    if isinstance(peer, int) and peer > 0:
        return int(f'-100{peer}')
    return peer


def _channel_ids_match(left, right) -> bool:
    """Match Pyrogram's high-level and raw numeric forms of a channel ID."""
    try:
        left_id = int(left)
        right_id = int(right)
    ...
    if left_id == right_id:
        return True
    if left_id > 0:
        left_id = int(f'-100{left_id}')
    if right_id > 0:
        right_id = int(f'-100{right_id}')
...
async def resolve_leave_target(client, target: str):
    ...
    if isinstance(peer, int):
        ...
        async for dialog in client.get_dialogs():
            ...
            if _channel_ids_match(getattr(chat, 'id', None), peer):
...
async def resolve_chat_target(client, target: str):
    ...
    if isinstance(peer, int):
        async for dialog in client.get_dialogs():
            chat = getattr(dialog, 'chat', None)
            if _channel_ids_match(getattr(chat, 'id', None), peer):
                return _stable_chat_id(chat) or peer, None
        try:
            chat = await client.get_chat(peer)
        ...
        async with client:
            if action in {'dm', 'bulk_dm'}:
                ...
                if join_link:
                    ...
                    peer, error = await resolve_chat_target(
                        client, normalize_numeric_channel_target(target)
                    )
                    if error:
                        return {'ok': False, 'error': error, 'fatal': False}
